...
def _cursor_to_df(cursor: pyodbc.Cursor) -> pd.DataFrame:
    """Low-level implementation to collect query to pd.DataFrame.
...
def df_from_dataflow(
    ...
    if columns is not None:
        ...
        warnings.warn(
            "Consider passing `columns` to load only what you need.",
            stacklevel=2,
        )
        cols_string = "*"
    ...
    if limit is _sentinel:
        warnings.warn(
            (
                "No `limit` provided. Defaulting to 10 rows to avoid "
                "performance issues."
            ),
            stacklevel=2,
        )
        ...
        warnings.warn(
            "Loading the entire dataset. This may impact performance.",
            stacklevel=2,
        )
    ...
    # Execute query
    return df_from_sql(sql=query, connection=connection)
